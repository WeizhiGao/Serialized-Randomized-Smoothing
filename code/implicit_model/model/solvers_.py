...
def broyden(f, x0, threshold, eps=1e-3, stop_mode="rel", ls=False, name="unknown"):
    ...
    while nstep < threshold:
        ...
        for mode in ['rel', 'abs']:
            ...
            if mode == stop_mode:
                lowest_xest = mix_batch(mask_lowest, x_est.clone().detach(), lowest_xest)
                lowest_gx = mix_batch(mask_lowest, gx.clone().detach(), lowest_gx)
                lowest_xest = lowest_xest.view_as(x0).clone().detach()
                final_xest = mix_batch(mask_stop, x_est.clone().detach(), final_xest)
...
def naive(f, x0, threshold=50, eps=1e-3, stop_mode='rel', indexing=None,  **kwargs):
    ...
    for k in range(threshold):
        ...
        for mode in ['rel', 'abs']:
            ...
            mask_lowest = diff_dict[mode] < lowest_dict[mode]
            if mode == stop_mode:
                lowest_xest = mix_batch(mask_lowest, fx.view_as(x0).clone().detach(), lowest_xest)
                lowest_xest = lowest_xest.view_as(x0).clone().detach()
                final_xest = mix_batch(mask_stop, fx.view_as(x0).clone().detach(), final_xest)
                final_xest = final_xest.view_as(x0).clone().detach()
